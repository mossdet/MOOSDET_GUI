...
public:
	...
	bool getSupportVectors();
	bool getEEG_EventsFromDetectedEpochs(unsigned sharedChs, std::string channName);

	void readEOI_VisualAnnotations(std::string channelName, int idNumber);
	void readEOI_VisualSpikes(std::string channelName, int idNumber);
	unsigned deleteRepeatedMarks(double maxOnsetDiff, double minSharedPercent);
	unsigned joinCoOccMarks(double maxOnsetDiff, double minSharedPercent, bool join);
	unsigned joinCoOccDetections(double maxOnsetDiff, double minSharedPercent, bool join);
	void getExpertAgreement(double minSharedPercent);


	bool generateDetectionsFile(double onsetMinDiff, double minSharedPercent);
	bool generateSmallDetectionsFile(double onsetMinDiff, double minSharedPercent);
	bool generateDetectionsAndFeaturesFile(double onsetMinDiff, double minSharedPercent);
	bool generateMarksFile(double onsetMinDiff, double minSharedPercent);
	bool generateJoinedMarksFile(double onsetMinDiff, double minSharedPercent);
	bool getSpikesWith_WithoutHFOs();
	bool getFastTansientHFOs(double onsetMinDiff, double minSharedPercent);
	double getMultiPatientPerformance(double maxOnsetDiff, double minSharedPercent, std::string patientName,std::string eegChann , bool useWindowConfInterv);

	void spikeDetection(std::vector<bool> &allSpikesEpochVal, std::vector<bool> &spikeHFO_EpochVal, std::vector<bool> &onlySpike_EpochVal);

	bool getMetrics(double &sensitivity, double &precision, double &specificity, double &accuracy, double &negativePredictiveValue,
		double &kappa, double &weightedKappa, double& mattCC);
	...
	void setDetectedEOI(std::vector<EOI_Event> &detectedEOI) {
		m_detectedEOI.clear();
		for (long idx = 0; idx < detectedEOI.size(); idx++) {
			m_detectedEOI.push_back(detectedEOI[idx]);
		}
	}
	...
	void setFileStartDateAndTime (DateAndTime fileStartDateAndTime){ m_fileStartDateAndTime  = fileStartDateAndTime; }
	 ...
	 bool writeFeaturesAndOutput(std::string channName);

	 std::string to_string_with_precision(float a_value, int preVal);
