...
//#define MEASURE_BIOMARKER_VALUE
//#define OCURRENCE_RATE


int compareDetectionEvent(EOI_Event a, EOI_Event b);

using namespace System::Runtime::InteropServices;
using namespace System::Threading::Tasks;

HFO_Detector::HFO_Detector(System::String^ strInputFileName, int fileType, System::String^ strDirName, int patIdx, int montage, std::vector<std::string> *selectedUnipolarChanels, std::vector<std::string> *selectedBipolarChanels, 
	std::string &eoiName, bool useChannsFileBool, System::Double startTime, System::Double endTime, bool compareDetectionsWithVisualMarks, int processorGroup, int processingPercent, int readMinutesEpochLenghth, bool useScalpEEG) {
	...
	m_readMinutesEpoch = readMinutesEpochLenghth;

	m_compareDetectionsWithVisualMarks = compareDetectionsWithVisualMarks;
	...
	maxNumThreads = maxNumThreads * ((double)processingPercent / 75.0);
...
int HFO_Detector::characterizeEEG() {
	...
	if (m_fileType == 1) {
		...
		size_t sampleType = 0;
		pReader = new IO_CoherenceBinSignalImporter(m_strFileName, sampleType, m_selectedUnipolarChanels, m_selectedBipolarChanels, m_useScalpEEG);	// selected channels vectors are empty
	} 
	else if (m_fileType == 4) {
		pReader = new BrandtBinaryReader(m_strFileName, m_selectedUnipolarChanels, m_selectedBipolarChanels, m_useScalpEEG);	// selected channels vectors are empty
	}
	else {
		return -3;
	}
	...
	m_nrChannels = m_useMontage ? m_montageLabels.size() : m_unipolarLabels.size();
	DateAndTime eegStartDate = pReader->getStartDate();
	double eegDuration = pReader->getSampleCount() / pReader->getSamplingRate();
	//saveFileEEG_StartEndTimes(eegStartDate, eegDuration);
	//return m_totalSampleCount;
	...
	/*******************************************************************************************************************************************************************************************************************/
	if ((m_analysisStartTime*m_samplingRate) > pReader->getSampleCount())
		m_analysisStartTime = (double)pReader->getSampleCount() / m_samplingRate;
	if ((m_analysisEndTime*m_samplingRate) > pReader->getSampleCount())
		m_analysisEndTime = (double)pReader->getSampleCount() / m_samplingRate;

	m_fileAnalysisStartSample = (m_analysisStartTime * m_samplingRate) > 0 ? (m_analysisStartTime * m_samplingRate) : 0;
	m_fileAnalysisSamplesToRead = (m_analysisEndTime - m_analysisStartTime) * m_samplingRate;// pReader->getSampleCount();
	if (m_fileAnalysisSamplesToRead > pReader->getSampleCount())
		m_fileAnalysisSamplesToRead = pReader->getSampleCount();

	m_totalSampleCount = m_fileAnalysisSamplesToRead;
	m_fileStartDateAndTime = pReader->getDateAndTime();

	long long bigUnreadSamples = m_fileAnalysisSamplesToRead;
	m_readCycleSamplesToRead = m_samplingRate * 60 * m_readMinutesEpoch;													//Detection and data read performed every 120 min, m_fileAnalysisSamplesToRead
	...
	while (bigUnreadSamples > 0) {
		...
		if (m_useMontage) {
			bool ok = pReader->ReadReformattedSamples(m_readCycleStartSample, m_readCycleSamplesToRead, m_reformattedSignalSamples);
			if (!ok)
				return 0;
		}
		else {
			bool ok = pReader->ReadSamples(m_readCycleStartSample, m_readCycleSamplesToRead, m_reformattedSignalSamples);
			if (!ok)
				return 0;
		}
		...
		m_normLength = 1*60;	//Normalization every 10 s, 1*60.0
#ifdef OCURRENCE_RATE
		m_normLength = 60;
#endif
		long processCycleSampleCount = readCycleReadSamples;
		long samplesToProcess = m_normLength * m_samplingRate <= processCycleSampleCount ? m_normLength * m_samplingRate : processCycleSampleCount;
...
#pragma omp parallel for
			for (int ch = 0; ch < m_reformattedSignalSamples.size(); ++ch) {
				for (long s = cycleReadStart; s < cycleReadEnd; ++s) {
					double val = m_reformattedSignalSamples[ch][s]; // CALGARY ? m_reformattedSignalSamples[ch][s] * -1 : m_reformattedSignalSamples[ch][s];
			...
			/*for (int ch = 0; ch < m_reformattedSignalSamples.size(); ++ch) {
				...
				//plotSelectedEEG_Channel(firstSampleToProcess, rewind, overlap, cycleReformattedSamples, antiOffset_plus_notch, rippleSelctdChs, fastRippleSelectedChs, ch);
			}*/
			/*int ch = 0;
			plotSelectedEEG_Channel(firstSampleToProcess, rewind, overlap, cycleReformattedSamples, antiOffset_plus_notch, rippleSelctdChs, fastRippleSelectedChs, ch);
			ch = 1;
...
#pragma omp section
			...
			//executionTimeLog(loopNr, firstSampleToProcess, samplesToProcess, unprocessedSamples, processCycleSampleCount, durationMs);
			...
			//generateEpochCharacterizationRipples(signalFeatures);
			//generateEpochCharacterizationFastRipples(signalFeatures);
			//generateEpochCharacterizationSpikes(signalFeatures);
			//generateEpochCharacterizationSpikesHFO(signalFeatures);

			unprocessedSamples -= samplesToProcess;
			firstSampleToProcess += samplesToProcess;
			if (unprocessedSamples < samplesToProcess) {
				samplesToProcess = unprocessedSamples;
			}
		}

		bigUnreadSamples -= readCycleReadSamples;
		m_readCycleStartSample += readCycleReadSamples;
		if (readCycleReadSamples > bigUnreadSamples) {
			readCycleReadSamples = bigUnreadSamples;
		}
	}
...
bool HFO_Detector::detect(EOI_Features &signalFeatures, long firstLocalSampleToRead) {
	...
	//if((m_minutesAnalyzed % 30) == 0)
#ifdef OCURRENCE_RATE
	setBiomarkerOccChannels();		//reset the occurrence-rates every minute
#endif
#ifdef MEASURE_BIOMARKER_VALUE
	if (m_patientName.find("NR_2000Hz_") != std::string::npos) {
		readDetectionsForSections();
	}
#endif
	...
	double minSharedPercent = 0.1; 0.1; 0.75;
	bool joinVisualMarks = true;
	bool useWindowConfInterv = (m_patientName.find("SNR0dB") != std::string::npos) || (m_patientName.find("SNR5dB") != std::string::npos) || (m_patientName.find("SNR10dB") != std::string::npos) || (m_patientName.find("SNR15dB") != std::string::npos);
	...
	for (int channIdx = 0; channIdx < nrChanns; ++channIdx) {
		...
		//std::string rippleFunctionName = m_exePath + "/decisionFunctions/Ripple_AR_1.6e-05_0.0008_4-24, 22_27_27.dat";
		...
		//std::string fastRippleFunctionName = m_exePath + "/decisionFunctions/FastRipple_AR_0.001_1e-09_4-24, 22_51_18.dat";
		...
		//NAR
		/*std::vector<unsigned> rippleSelectedFeatures{ 4, 5, 14, 22, 28, 36, 40, 41, 42, 43, 45, 46, 48, 49, 50 };
		//std::string rippleFunctionName = m_exePath + "/decisionFunctions/decisionFunction_0.0001_0.0001_5-1, 15_27_11.dat";
		std::string rippleFunctionName = m_exePath + "/decisionFunctions/SVM_Ripple_NAR.dat";
		double rippleTh = 0;
		std::vector<unsigned> fastRippleSelectedFeatures{ 8, 9, 14, 22, 28, 36, 54, 55, 56, 57, 64 };
		//std::string fastRippleFunctionName = m_exePath + "/decisionFunctions/decisionFunction_1e-09_0.001_4-25, 0_33_20.dat";
		std::string fastRippleFunctionName = m_exePath + "/decisionFunctions/SVM_FR_NAR.dat";
		double fastRippleTh = 5;*/

		std::vector<unsigned> spikeSelectedFeatures{ 12, 14, 15, 23, 24, 26, 28, 29, 31, 37 };
		//std::string spikeFunctionName = m_exePath + "/decisionFunctions/IES_PFSA80_NPR1_1e-05_1e-05_3-8, 5_27_31.dat";
		std::string spikeFunctionName = m_exePath + "/svm_ies.dat";
		double spikeTh = 0.0;

#ifndef DETECT_ONLY_HFO
#pragma omp parallel sections
		{
#pragma omp section
			{
#endif // !DETECT_ONLY_HFO

				//All HFO Detection
				if (m_eoiName.compare("HFO") == 0) {

					SVM_Detector svmHFO_Detector("AllHFO", m_patientName, rippleFunctionName, "", rippleSelectedFeatures, m_outputDirectory);
					...
					//svmHFO_Detector.generateDetectionsFile(maxOnsetDiff, minSharedPercent);
					//svmHFO_Detector.generateSmallDetectionsFile(maxOnsetDiff, minSharedPercent);
					...
					// Mixed events detection
					std::vector<EOI_Event> readAllMixedEvents;
					readAllMixedEvents.insert(readAllMixedEvents.end(), readRippleEvents.begin(), readRippleEvents.end());
					readAllMixedEvents.insert(readAllMixedEvents.end(), readFastRippleEvents.begin(), readFastRippleEvents.end());
					if (!getPerformance()) {
						readAllMixedEvents.insert(readAllMixedEvents.end(), readSpikeEvents.begin(), readSpikeEvents.end());
					}
					std::sort(readAllMixedEvents.begin(), readAllMixedEvents.end(), compareDetectionEvent);
					svmHFO_Detector.setDetectedEOI(readAllMixedEvents);
					svmHFO_Detector.joinCoOccDetections(maxOnsetDiff, minSharedPercent, true);
					//svmHFO_Detector.joinCoOccDetections(maxOnsetDiff, minSharedPercent, false);
					svmHFO_Detector.getEvents(readAllMixedEvents);
					svmHFO_Detector.setEOI_Name("HFO");

					if (getPerformance()) {
						svmHFO_Detector.readEOI_VisualAnnotations(channelName, idNumber);
						svmHFO_Detector.deleteRepeatedMarks(maxOnsetDiff, minSharedPercent);
						//svmHFO_Detector.getExpertAgreement(minSharedPercent);
						svmHFO_Detector.generateMarksFile(maxOnsetDiff, minSharedPercent);
						svmHFO_Detector.joinCoOccMarks(maxOnsetDiff, minSharedPercent, joinVisualMarks);
						//svmHFO_Detector.joinCoOccMarks(maxOnsetDiff, minSharedPercent, false);
						svmHFO_Detector.generateJoinedMarksFile(maxOnsetDiff, minSharedPercent);
						svmHFO_Detector.readEOI_VisualSpikes(channelName, idNumber);
					}
					//svmHFO_Detector.generateDetectionsFile(maxOnsetDiff, minSharedPercent);
					//svmHFO_Detector.generateSmallDetectionsFile(maxOnsetDiff, minSharedPercent);
					svmHFO_Detector.generateDetectionsAndFeaturesFile(maxOnsetDiff, minSharedPercent);
					...
					//svmHFO_Detector.getFastTansientHFOs(maxOnsetDiff, minSharedPercent);
					if (getPerformance()) {
						if (svmHFO_Detector.isChannelAnnotated()) {
							svmHFO_Detector.getMultiPatientPerformance(maxOnsetDiff, minSharedPercent, m_patientName, channelName, useWindowConfInterv);
						}
					}
#ifdef MEASURE_BIOMARKER_VALUE
					getChannelsHFO_Statistics(channelName, readRippleEvents, readFastRippleEvents, readSpikeEvents, readAllMixedEvents);
#endif
#ifdef OCURRENCE_RATE
					getBiomarkerOcurrenceRates(channIdx, readRippleEvents, readFastRippleEvents, readSpikeEvents, readAllMixedEvents);
#endif
				}
#ifndef DETECT_ONLY_HFO
			}
#pragma omp section
			{
				//Ripple Detection
				if (m_eoiName.compare("HFO") == 0 || m_eoiName.compare("Ripples") == 0) {

					SVM_Detector svmRipple_Detector("AllHFO", m_patientName, rippleFunctionName, "", rippleSelectedFeatures, m_outputDirectory);
					svmRipple_Detector.setEOI_Name("Ripple");
					svmRipple_Detector.setFunctionName(rippleFunctionName);
					svmRipple_Detector.setProbThreshold(0, rippleTh);
					svmRipple_Detector.setPatientPath(m_patientPath);

					svmRipple_Detector.initEpochVectors(1);
					svmRipple_Detector.setInputEvents(inputEpochs);
					svmRipple_Detector.setAbsoluteSignalLength(m_totalSampleCount / m_samplingRate);
					//Detect channel
					svmRipple_Detector.detectEpochs(0);
					svmRipple_Detector.getEEG_EventsFromDetectedEpochs(1, channelName);
					svmRipple_Detector.setSamplingRate(m_samplingRate);
					svmRipple_Detector.setCI(useWindowConfInterv);
					svmRipple_Detector.setFileStartDateAndTime(m_fileStartDateAndTime);

					if (getPerformance()) {
						svmRipple_Detector.readEOI_VisualAnnotations(channelName, idNumber);
						svmRipple_Detector.deleteRepeatedMarks(maxOnsetDiff, minSharedPercent);
						//svmRipple_Detector.getExpertAgreement(minSharedPercent);
						svmRipple_Detector.generateMarksFile(maxOnsetDiff, minSharedPercent);

						svmRipple_Detector.joinCoOccMarks(maxOnsetDiff, minSharedPercent, joinVisualMarks);
						svmRipple_Detector.generateJoinedMarksFile(maxOnsetDiff, minSharedPercent);

						svmRipple_Detector.readEOI_VisualSpikes(channelName, idNumber);
					}
					svmRipple_Detector.generateDetectionsFile(maxOnsetDiff, minSharedPercent);
					svmRipple_Detector.generateDetectionsAndFeaturesFile(maxOnsetDiff, minSharedPercent);
					//svmRipple_Detector.getFastTansientHFOs(maxOnsetDiff, minSharedPercent);

					if (getPerformance()) {
						if (svmRipple_Detector.isChannelAnnotated()) {
							svmRipple_Detector.getMultiPatientPerformance(maxOnsetDiff, minSharedPercent, m_patientName, channelName, useWindowConfInterv);
						}
					}
				}
			}
...
#pragma omp section
			{
				//Fast-Ripple Detection
				if (m_eoiName.compare("HFO") == 0 || m_eoiName.compare("FastRipples") == 0) {

					SVM_Detector svmFastRippleDetector("FastRipple", m_patientName, fastRippleFunctionName, "", fastRippleSelectedFeatures, m_outputDirectory);
					svmFastRippleDetector.setEOI_Name("FastRipple");
					svmFastRippleDetector.setProbThreshold(0, fastRippleTh);
					svmFastRippleDetector.setPatientPath(m_patientPath);

					svmFastRippleDetector.initEpochVectors(1);
					svmFastRippleDetector.setInputEvents(inputEpochs);
					svmFastRippleDetector.setAbsoluteSignalLength(m_totalSampleCount / m_samplingRate);

					//Detect channel
					svmFastRippleDetector.detectEpochs(0);
					svmFastRippleDetector.getEEG_EventsFromDetectedEpochs(1, channelName);
					svmFastRippleDetector.setSamplingRate(m_samplingRate);
					svmFastRippleDetector.setCI(useWindowConfInterv);
					svmFastRippleDetector.setFileStartDateAndTime(m_fileStartDateAndTime);

					if (getPerformance()) {
						svmFastRippleDetector.readEOI_VisualAnnotations(channelName, idNumber);
						svmFastRippleDetector.deleteRepeatedMarks(maxOnsetDiff, minSharedPercent);
						//svmFastRippleDetector.getExpertAgreement(minSharedPercent);
						svmFastRippleDetector.generateMarksFile(maxOnsetDiff, minSharedPercent);

						svmFastRippleDetector.joinCoOccMarks(maxOnsetDiff, minSharedPercent, joinVisualMarks);
						svmFastRippleDetector.generateJoinedMarksFile(maxOnsetDiff, minSharedPercent);

						svmFastRippleDetector.readEOI_VisualSpikes(channelName, idNumber);
					}

					svmFastRippleDetector.generateDetectionsFile(maxOnsetDiff, minSharedPercent);
					svmFastRippleDetector.generateDetectionsAndFeaturesFile(maxOnsetDiff, minSharedPercent);
					//svmFastRippleDetector.getFastTansientHFOs(maxOnsetDiff, minSharedPercent);

					if (getPerformance()) {
						if (svmFastRippleDetector.isChannelAnnotated()) {
							svmFastRippleDetector.getMultiPatientPerformance(maxOnsetDiff, minSharedPercent, m_patientName, channelName, useWindowConfInterv);
						}
					}
				}
			}
#pragma omp section
			{
				//Spikes Detection
				if (m_eoiName.compare("HFO") == 0 || m_eoiName.compare("Spikes") == 0) {
					//if (m_eoiName.compare("Spikes") == 0) {
					SVM_Detector svmSpikeDetector("Spike", m_patientName, spikeFunctionName, "", spikeSelectedFeatures, m_outputDirectory);
					svmSpikeDetector.setEOI_Name("Spike");
					svmSpikeDetector.setProbThreshold(0, spikeTh);
					svmSpikeDetector.setPatientPath(m_patientPath);

					svmSpikeDetector.initEpochVectors(1);
					svmSpikeDetector.setInputEvents(inputEpochs);
					svmSpikeDetector.setAbsoluteSignalLength(m_totalSampleCount / m_samplingRate);

					//Detect channel
					svmSpikeDetector.detectEpochs(0);
					svmSpikeDetector.getEEG_EventsFromDetectedEpochs(1, channelName);
					svmSpikeDetector.setSamplingRate(m_samplingRate);
					svmSpikeDetector.setCI(useWindowConfInterv);
					svmSpikeDetector.setFileStartDateAndTime(m_fileStartDateAndTime);

					if (getPerformance()) {
						svmSpikeDetector.readEOI_VisualAnnotations(channelName, idNumber);
						svmSpikeDetector.deleteRepeatedMarks(maxOnsetDiff, minSharedPercent);
						//svmSpikeDetector.getExpertAgreement(minSharedPercent);
						svmSpikeDetector.generateMarksFile(maxOnsetDiff, minSharedPercent);

						svmSpikeDetector.joinCoOccMarks(maxOnsetDiff, minSharedPercent, joinVisualMarks);
						svmSpikeDetector.generateJoinedMarksFile(maxOnsetDiff, minSharedPercent);

						svmSpikeDetector.readEOI_VisualSpikes(channelName, idNumber);
					}

					svmSpikeDetector.generateDetectionsFile(maxOnsetDiff, minSharedPercent);
					svmSpikeDetector.generateDetectionsAndFeaturesFile(maxOnsetDiff, minSharedPercent);

					if (getPerformance()) {
						if (svmSpikeDetector.isChannelAnnotated()) {
							svmSpikeDetector.getMultiPatientPerformance(maxOnsetDiff, minSharedPercent, m_patientName, channelName, useWindowConfInterv);
						}
					}
				}
			}
	...
	//svmDetector.readFeaturesFromFile(patientDataTest[patIdx].DataFilenames[chIdx], false);

#ifdef MEASURE_BIOMARKER_VALUE
	if (m_patientName.find("NR_2000Hz_") != std::string::npos) {
		writeDetectionsForSections();
	}
	getROCs(true);
	getROCs(false);
#endif

	//getPercentageROCs(true);
	//getPercentageROCs(false);

	bool biomarkerStatsOk = true;
#ifdef OCURRENCE_RATE
	biomarkerStatsOk = generateFileBiomarkersStatistics();
#endif

	return biomarkerStatsOk;
}
...
#pragma omp parallel for
	for (int channIdx = 0; channIdx < nrChannsToAnalyze; ++channIdx) {
		...
		double epochSubHFO_SpectralPeak = 0, epochSubHFO_MaxSpindleRangePower = 0;
		double epochRippleSpectralPeak = 0, epochRippleMaxSpindleRangePower = 0;
		double epochFastRippleSpectralPeak = 0, epochFastRippleMaxSpindleRangePower = 0;
...
void HFO_Detector::saveFileEEG_StartEndTimes(DateAndTime eegStartDate, double eegDuration) {
	std::ofstream eegStartEnd;

	std::string outputPath = m_outputDirectory + "/MOSSDET_Output";
	std::string patPath = outputPath + "/" + m_patientName;
	std::string filename = patPath + "/" + m_patientName + "_eegStartEnd.txt";

	mkdir(outputPath.c_str());
	mkdir(patPath.c_str());
	remove(filename.c_str());

	eegStartEnd.open(filename);
	eegStartEnd << "Duration(s)\t";
	eegStartEnd << "sYear\t" << "sMonth\t" << "sDay\t" << "sHour\t" << "sMin\t" << "sSec\t" << "sSubs\n";
	//eegStartEnd << "eYear\t" << "eMonth\t" << "eDay\t" << "eHour\t" << "eMin\t" << "eSec\t" << "eSubs\n";

	eegStartEnd << eegDuration << "\t";
	eegStartEnd << eegStartDate.year << "\t" << eegStartDate.month << "\t" << eegStartDate.day << "\t";
	eegStartEnd << eegStartDate.hours << "\t" << eegStartDate.minutes << "\t" << eegStartDate.seconds << "\t" << eegStartDate.milliSeconds << "\n";
...
bool HFO_Detector::plotSelectedEEG_Channel(long firstSampleToProcess, unsigned rewind, unsigned overlap, matrixStd& signal, matrixStd& notchedSignal, matrixStd& rippleSignal, matrixStd& fastRippleSignal, unsigned channelToSave) {
	...
	mkdir("FilterCharacterization/");
	mkdir(patientPath.c_str());
...
void HFO_Detector::getChannelsHFO_Statistics(std::string channelName, std::vector<EOI_Event> &readRippleEvents, std::vector<EOI_Event> &readFastRippleEvents, std::vector<EOI_Event> &readSpikeEvents, std::vector<EOI_Event> &readAllMixedEvents) {
	...
	long nrDetections = readAllMixedEvents.size();
	for (int ch = 0; ch < nrAnalyzedChannels; ++ch) {
		if (m_channelsHFO_Statistics[ch].channelName.compare(channelName) == 0) {	// find right channel

			m_channelsHFO_Statistics[ch].totalRipples += readRippleEvents.size();
			m_channelsHFO_Statistics[ch].totalFRs += readFastRippleEvents.size();
			m_channelsHFO_Statistics[ch].totalHFO += readRippleEvents.size() + readFastRippleEvents.size();
			m_channelsHFO_Statistics[ch].totalSpikes += readSpikeEvents.size();

			for (long detIdx = 0; detIdx < nrDetections; ++detIdx) {				// read detection
				std::string label = readAllMixedEvents[detIdx].description;
				bool foundFastRipple = label.find("ast") != std::string::npos;
				bool foundRipple = ((label.find("ipple") != std::string::npos) || (label.find("HFO") != std::string::npos)) && !foundFastRipple;
				bool foundRippleAndFastRipple = false;
				bool foundSpike = label.find("ike") != std::string::npos;
				...
				// isolatedRipples, isolatedFRs, isolatedHFO
				if (foundRipple && (!foundFastRipple && !foundRippleAndFastRipple && !foundSpike)) {
					m_channelsHFO_Statistics[ch].totalIsolatedRipple++;
					m_channelsHFO_Statistics[ch].totalIsolatedHFO++;
				}
				if (foundFastRipple && (!foundRipple && !foundRippleAndFastRipple && !foundSpike)) {
					m_channelsHFO_Statistics[ch].totalIsolatedFR++;
					m_channelsHFO_Statistics[ch].totalIsolatedHFO++;
				}
				if (foundRippleAndFastRipple && (!foundRipple && !foundFastRipple && !foundSpike)) {
					m_channelsHFO_Statistics[ch].totalIsolatedRipple++;
					m_channelsHFO_Statistics[ch].totalIsolatedFR++;
					m_channelsHFO_Statistics[ch].totalIsolatedHFO += 2;
				}
			}
			...
			m_channelsHFO_Statistics[ch].isolatedRippleRatePerMinute = (double)m_channelsHFO_Statistics[ch].totalIsolatedRipple / (double)m_channelsHFO_Statistics[ch].nrMinutes;
			m_channelsHFO_Statistics[ch].isolatedFrRatePerMinute = (double)m_channelsHFO_Statistics[ch].totalIsolatedFR / (double)m_channelsHFO_Statistics[ch].nrMinutes;
			m_channelsHFO_Statistics[ch].isolatedHfoRatePerMinute = (double)m_channelsHFO_Statistics[ch].totalIsolatedHFO / (double)m_channelsHFO_Statistics[ch].nrMinutes;
...
void HFO_Detector::getROCs(bool localizeResectionOrSOZ) {
	...
	for (int paramIdx = 1; paramIdx <= 19; ++paramIdx) {
		...
		if (paramIdx == 13) paramName = "isolatedRippleRatePerMinute";
		if (paramIdx == 14) paramName = "isolatedFrRatePerMinute";
		if (paramIdx == 15) paramName = "isolatedHfoRatePerMinute";
		...
		if (paramIdx == 19) paramName = "spindleRippleRatePerMinute";
		if (paramIdx == 20) paramName = "spindleFrRatePerMinute";
		if (paramIdx == 21) paramName = "spindleHfoRatePerMinute";
		...
		for (int ch = 0; ch < nrAnalyzedChannels; ++ch) {
			...
			if (paramIdx == 13) parameterVal = m_channelsHFO_Statistics[ch].isolatedRippleRatePerMinute;
			if (paramIdx == 14) parameterVal = m_channelsHFO_Statistics[ch].isolatedFrRatePerMinute;
			if (paramIdx == 15) parameterVal = m_channelsHFO_Statistics[ch].isolatedHfoRatePerMinute;
			...
			if (paramIdx == 19) parameterVal = m_channelsHFO_Statistics[ch].spindleRippleRatePerMinute;
			if (paramIdx == 20) parameterVal = m_channelsHFO_Statistics[ch].spindleFrRatePerMinute;
			if (paramIdx == 21) parameterVal = m_channelsHFO_Statistics[ch].spindleHfoRatePerMinute;
...
if (paramIdx == 13) parameterVal = m_channelsHFO_Statistics[ch].isolatedRippleRatePerMinute / maxAcrossChanns;
if (paramIdx == 14) parameterVal = m_channelsHFO_Statistics[ch].isolatedFrRatePerMinute / maxAcrossChanns;
if (paramIdx == 15) parameterVal = m_channelsHFO_Statistics[ch].isolatedHfoRatePerMinute / maxAcrossChanns;
...
void HFO_Detector::writeDetectionsForSections() {
	...
	sectionsFile << "nrMinutes" << "\t" << "ChannelName" << "\t" << "totalRipples" << "\t" << "totalFRs" << "\t" << "totalHFO" << "\t" << "totalSpikes" << "\t"
		<< "totalIsolatedRipple" << "\t" << "totalIsolatedFR" << "\t" << "totalIsolatedHFO" << "\t"
		<< "totalSpikeAndRipple" << "\t" << "totalSpikeAndFR" << "\t" << "totalSpikeAndHFO" << "\t"
	...
	for (int ch = 0; ch < nrAnalyzedChannels; ++ch) {
		sectionsFile << m_channelsHFO_Statistics[ch].nrMinutes << "\t" << m_channelsHFO_Statistics[ch].channelName << "\t"
			...
			<< m_channelsHFO_Statistics[ch].totalIsolatedRipple << "\t" << m_channelsHFO_Statistics[ch].totalIsolatedFR << "\t" << m_channelsHFO_Statistics[ch].totalIsolatedHFO << "\t"
			<< m_channelsHFO_Statistics[ch].totalSpikeAndRipple << "\t" << m_channelsHFO_Statistics[ch].totalSpikeAndFR << "\t" << m_channelsHFO_Statistics[ch].totalSpikeAndHFO << "\t"
...
void HFO_Detector::readDetectionsForSections() {
	...
	if (fileExists(fn.c_str())) {
		...
		while (!sectionsFile.eof()) {
			...
			ss >> readSavedChannelAndResectionData.totalIsolatedRipple;
			ss >> readSavedChannelAndResectionData.totalIsolatedFR;
			ss >> readSavedChannelAndResectionData.totalIsolatedHFO;
			...
			if (!readSavedChannelAndResectionData.channelName.empty()) {
				...
				for (int ch = 0; ch < nrAnalyzedChannels; ++ch) {
					...
					if (selChannName.compare(channelName) == 0) {	// is channel from resection file found within the channels from the EEG file?
						...
						m_channelsHFO_Statistics[ch].totalIsolatedRipple = readSavedChannelAndResectionData.totalIsolatedRipple;
						m_channelsHFO_Statistics[ch].totalIsolatedFR = readSavedChannelAndResectionData.totalIsolatedFR;
						m_channelsHFO_Statistics[ch].totalIsolatedHFO = readSavedChannelAndResectionData.totalIsolatedHFO;
...
bool HFO_Detector::writePerChannelDetectionsFile(std::string directory, std::string eventType, std::string channName, std::vector<EOI_Event> &detections) {
	...
	mkdir(detectionTypeDirectory.c_str());
...
void HFO_Detector::getBiomarkerOcurrenceRates(int channIdx, std::vector<EOI_Event> &readRippleEvents, std::vector<EOI_Event> &readFastRippleEvents, std::vector<EOI_Event> &readSpikeEvents, std::vector<EOI_Event> &readAllMixedEvents) {

	long nrDetections = readAllMixedEvents.size();

	m_biomarkerOccPerChann[channIdx].totalRipples += readRippleEvents.size();
	m_biomarkerOccPerChann[channIdx].totalFRs += readFastRippleEvents.size();
	m_biomarkerOccPerChann[channIdx].totalHFO += readRippleEvents.size() + readFastRippleEvents.size();
	m_biomarkerOccPerChann[channIdx].totalSpikes += readSpikeEvents.size();
	...
	for (long detIdx = 0; detIdx < nrDetections; ++detIdx) {				// read detection, all of these detection correspond to the channel signaled by channIdx
		...
		// isolatedRipples, isolatedFRs, isolatedHFO
		if (foundRipple && (!foundFastRipple && !foundRippleAndFastRipple && !foundSpike)) {
			m_biomarkerOccPerChann[channIdx].totalIsolatedRipple++;
			m_biomarkerOccPerChann[channIdx].totalIsolatedHFO++;
		}
		if (foundFastRipple && (!foundRipple && !foundRippleAndFastRipple && !foundSpike)) {
			m_biomarkerOccPerChann[channIdx].totalIsolatedFR++;
			m_biomarkerOccPerChann[channIdx].totalIsolatedHFO++;
		}
		if (foundRippleAndFastRipple && (!foundRipple && !foundFastRipple && !foundSpike)) {
			m_biomarkerOccPerChann[channIdx].totalIsolatedRipple++;
			m_biomarkerOccPerChann[channIdx].totalIsolatedFR++;
			m_biomarkerOccPerChann[channIdx].totalIsolatedHFO += 2;
		}
	}
	...
	m_biomarkerOccPerChann[channIdx].isolatedRippleRatePerMinute = (double)m_biomarkerOccPerChann[channIdx].totalIsolatedRipple / (double)m_biomarkerOccPerChann[channIdx].nrMinutes;
	m_biomarkerOccPerChann[channIdx].isolatedFrRatePerMinute = (double)m_biomarkerOccPerChann[channIdx].totalIsolatedFR / (double)m_biomarkerOccPerChann[channIdx].nrMinutes;
	m_biomarkerOccPerChann[channIdx].isolatedHfoRatePerMinute = (double)m_biomarkerOccPerChann[channIdx].totalIsolatedHFO / (double)m_biomarkerOccPerChann[channIdx].nrMinutes;
...
bool HFO_Detector::generateFileBiomarkersStatistics() {
	...
	if (!fileExists(filename.c_str())) {
		...
		biomarkerRatesFile << "Channel\t" << "DateAndTime\t" << "DayNr\t" << "EpochNr\t" << "EpochLength(s)\t" << "Ripples/Epoch\t" << "FR/Epoch\t" << "HFO/Epoch\t" << "IES/Epoch\t" << "IES-Ripples/Epoch\t" << "IES-FR/Epoch\t" << "IES-HFO/Epoch\t" << "isolatedRipples/Epoch\t" << "isolatedFR/Epoch\t" << "isolatedHFO/Epoch\n";
	}
	else {
	...
	for (int ch = 0; ch < m_nrChannels; ++ch) {
		...
		biomarkerRatesFile << m_normLength << "\t";
		...
		biomarkerRatesFile << m_biomarkerOccPerChann[ch].isolatedRippleRatePerMinute << "\t";
		biomarkerRatesFile << m_biomarkerOccPerChann[ch].isolatedFrRatePerMinute << "\t";
		biomarkerRatesFile << m_biomarkerOccPerChann[ch].isolatedHfoRatePerMinute << "\n";
