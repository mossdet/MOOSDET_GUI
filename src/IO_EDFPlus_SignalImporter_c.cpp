...
IO_EDFPlus_SignalImporter_c::IO_EDFPlus_SignalImporter_c(const std::string strFilename, std::vector<std::string > selectedUnipolarChanns, std::vector<std::string > selectedBipolarChanns, bool useScalpEEG):
...
bool IO_EDFPlus_SignalImporter_c::readHeader()
{
	...
	/*int *edf_error;
	FILE *inputfile;
	edfhdrblock *readBlock = edflib_check_edf_file(inputfile, edf_error);*/
	...
	_int64 sampleCount = edfhdr.signalparam[0].smp_in_file;
	...
	for (unsigned int n = 0; n < nSignals; n++) {
		...
		std::string channNameStr = contact.electrodeName + std::to_string(contact.contactNr);
		if (!useTenTwentySystem && (std::find(tenTwentyChannels.begin(), tenTwentyChannels.end(), channNameStr) != tenTwentyChannels.end())) {
			continue;
		}
...
double IO_EDFPlus_SignalImporter_c::getSamplingRate() {
...
// sample count
unsigned long IO_EDFPlus_SignalImporter_c::getSampleCount(){
...
unsigned int IO_EDFPlus_SignalImporter_c::getNumberUnipolarChannels(){
	return (unsigned int)m_unipolarContacts.size();
}

unsigned int IO_EDFPlus_SignalImporter_c::getNumberReformattedMontages(){
